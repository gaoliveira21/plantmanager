...
export async function savePlant(plant: Plant): Promise<void> {
  try {
    ...
    const noficationId = await Notifications.scheduleNotificationAsync({
      content: {
        title: 'Heeey, 🥗',
    ...
    const newPlant = {
      [plant.id]: { data: plant, noficationId },
    }
...
export async function removePlant(id: number): Promise<void> {
  ...
  await Notifications.cancelScheduledNotificationAsync(plants[id].noficationId)
